<template>
  <div>
    ...
    <!-- Page Content -->
    <div class="content">
      <base-block rounded title="Riders" ref="blockLoadingRefresh" class="block-mode-loading-refresh"  header-bg btn-option-fullscreen >
		<template #options>
    ...
    <!-- TABLE **************************-->
        <RecordTable :records="records">
			...
			<td>{{ rs.bike_no }}</td>
			<td>{{ rs.first_name }}</td>
			<td class="">{{ rs.last_name }}</td>
			<td class="">{{ rs.gender }}</td>
			<td class="">{{ rs.mobile }}</td>
			<td class="">{{ rs.ID_document_number }}</td>
			<td class="">{{ rs.date_registered }}</td>
			<td class="text-center">
				<button v-if="rs.status == 'Active'" class=" btn status-btn btn-success " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
				<button v-else-if="rs.status == 'Inactive'" class=" btn status-btn btn-primary " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
	  ...
	  <!-- modal -->
	  <OuModal mdId="add-record" size="lg" title="Add Riders">
		  <CreateRecord :getRecords="getRecords"/>
	  </OuModal>

	  <OuModal mdId="dmk-view-record" size="lg" title="Riders Details" :modalSpinner="record_spinner">
		  <RecordView :recordbyId="recordbyId" />
	  </OuModal>

	  <OuModal mdId="dmk-update-record" size="lg" title="Update Riders Details" :modalSpinner="record_spinner">
...
export default {
	...
	setLoadingFalse(){
        ...
        async getRecordOnMount(){
            ...
            await this.$axios.$get(".inc/v1/riders/mini")
            .then((resp) =>{
                this.records = resp;
       ...
       async getRecordById(id){
		   ...
		   let token = localStorage.getItem('token')
			this.$axios.defaults.headers.common["Authorization"] = "Token " + token
           await this.$axios.$get(`v1/motorbike/${id}`)	
            .then((resp) =>{
	...
	Swal.fire({
			...
			if (result.isConfirmed) {
				 this.$axios.$delete(`v1/motorbike/delete/${id}`)
					.then((resp) =>{
						if(resp.error == 'false'){
	...
	async searchRecords(){
	  ...
	  let token = localStorage.getItem('token')
	  this.$axios.defaults.headers.common["Authorization"] = "Token " + token
       await this.$axios.$get(`v1/motorbike/search/${this.search.queary}`)
           .then((resp) =>{
			   this.records = resp; 
           })
		   this.setLoadingFalse();

<template>
  <div>
    ...
    <!-- Page Content -->
    <div class="content">
      <base-block rounded title="Battery Stations" ref="blockLoadingRefresh" class="block-mode-loading-refresh"  header-bg btn-option-fullscreen >
		<template #options>
    ...
    <!-- TABLE **************************-->
        <RecordTable :records="records">
			...
			<td>{{ rs.title }}</td>
			<td class="text-center">{{ rs.head }}</td>
			...
			<td class="text-center">
				...
				<button v-else-if="rs.status == 'Construction'" class=" btn status-btn btn-primary " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
				<button v-else-if="rs.status == 'Closed'" class=" btn status-btn btn-warning " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
	  ...
	  <!-- modal -->
	  <OuModal mdId="add-record" size="md" title="Add Battery Station">
		  <CreateRecord :getRecords="getRecords"/>
	  </OuModal>

	  <OuModal mdId="dmk-view-record" size="md" title="Battery Station Details" :modalSpinner="record_spinner">
		  <RecordView :recordbyId="recordbyId" />
	  </OuModal>

	  <OuModal mdId="dmk-update-record" size="md" title="Update Battery Station Details" :modalSpinner="record_spinner">
...
export default {
	...
	Swal.fire({
			...
			if (result.isConfirmed) {
				 this.$axios.$delete(`v1/battery/station/delete/${id}`)
					.then((resp) =>{
						if(resp.error == 'false'){
							let swa_msg = 'Battery Station deleted successfully';

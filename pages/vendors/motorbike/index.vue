<template>
  <div>
    ...
    <!-- Page Content -->
    <div class="content">
      <base-block rounded title="Motorbikes" ref="blockLoadingRefresh" class="block-mode-loading-refresh"  header-bg btn-option-fullscreen >
		<template #options>
    ...
    <!-- TABLE **************************-->
        <RecordTable :records="records">
			...
			<td>{{ rs.memNo }}</td>
			<td class="">{{ rs.client }}</td>
			<td class="">{{ rs.numberplate }}</td>
			<td class="text-center">
				<button v-if="rs.status == 'Sold'" class=" btn status-btn btn-success " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
				<button v-else-if="rs.status == 'Leased'" class=" btn status-btn btn-primary " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
	  ...
	  <!-- modal -->
	  <OuModal mdId="add-record" size="md" title="Add Motorbike">
		  <CreateRecord :getRecords="getRecords"/>
	  </OuModal>

	  <OuModal mdId="dmk-view-record" size="md" title="Motorbike Details" :modalSpinner="record_spinner">
		  <RecordView :recordbyId="recordbyId" />
	  </OuModal>

	  <OuModal mdId="dmk-update-record" size="md" title="Update Motorbike Details" :modalSpinner="record_spinner">
...
export default {
	...
	setLoadingFalse(){
        ...
        async getRecordOnMount(){
            ...
            await this.$axios.$get("v1/motorbikes")
            .then((resp) =>{
                this.records = resp;

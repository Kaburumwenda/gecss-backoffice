<template>
    <div>
         <b-form @submit.prevent="get_mpesa_daily_pdf()">
           <div>
            <div uk-grid>
...
export default {
    ...
    methods:{
       async get_mpesa_daily_pdf(){	
            ...
            await this.$axios.$post('v1/reports/mpesa_statistics/pdf_excel', formData)
                .then((resp) =>{
                    ...
                    if( resp_count >= 1){
                        ...
                        let d3 = `mpesa_reports-pdf-${d1} - ${d2}.pdf`
                        ...
                        let info = []
                            result.forEach((element, index, array) => {
                                info.push(
                                    [ index + 1, element.Date, element.Month, element.Day, element.Year, element.Revenue,
                                    ])
                            })
                            ...
                            doc.autoTable({
                            head: [[ "No", "Date", "Month", "Day", "Year", "Revenue (KES)"]],
                            body: info,
                            });
                            // OTHER OPTIONS
                            // theme: 'grid',

...
export default {
    ...
    async generateExcel() {
        ...
        .then((resp) =>{
            ...
            if(resp_count >= 1){
                let d1 = this.mpesa_filter.fromdate
                let d2 = this.mpesa_filter.todate
                let d3 = `mpesa_reports-excel-${d1} - ${d2}.xlsx`
                const data = XLSX.utils.json_to_sheet(resp)
                const wb = XLSX.utils.book_new()
                XLSX.utils.book_append_sheet(wb, data, 'data')
                XLSX.writeFile(wb, d3)

                this.$bvModal.hide('mpesa-daily-excel-reports')
                ...
                this.$bvModal.hide('excel-mpesa-record')

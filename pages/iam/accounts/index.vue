<template>
  <div>
    ...
    <!-- Page Content -->
    <div class="content">
      <base-block rounded title="Customer Accounts" ref="blockLoadingRefresh" class="block-mode-loading-refresh"  header-bg btn-option-fullscreen >
		<template #options>
    ...
    <!-- TABLE **************************-->
        <RecordTable :records="records">
			...
			<td class="mobile-td-expand-120">{{ rs.memNo }}</td>
			<td>{{ rs.client }}</td>
			<td class="">{{ rs.idNo }}</td>
			<td>{{ rs.phone }}</td>
			<td>{{ rs.sex }}</td>
			<td>KES {{ rs.amount }}</td>
			<td>KES {{ rs.balance }}</td>
	  ...
	  <!-- modal -->
	  <OuModal mdId="add-record" size="lg" title="Add Customer Account">
		  <CreateRecord :getRecords="getRecords"/>
	  </OuModal>

	  <OuModal mdId="dmk-view-record" size="lg" title="Customer Account Details" :modalSpinner="record_spinner">
		  <RecordView :recordbyId="recordbyId" />
	  </OuModal>

	  <OuModal mdId="dmk-update-record" size="lg" title="Update Customer Account Details" :modalSpinner="record_spinner">
...
export default {
	...
	Swal.fire({
			...
			if (result.isConfirmed) {
				 this.$axios.$delete(`v1/account/delete/${id}`)
					.then((resp) =>{
						if(resp.error == 'false'){
							let swa_msg = 'Customer account deleted successfully';

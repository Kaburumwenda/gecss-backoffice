<template>
  <div>
    ...
    <!-- Page Content -->
    <div class="content">
      <base-block rounded title="Staff" ref="blockLoadingRefresh" class="block-mode-loading-refresh"  header-bg btn-option-fullscreen >
		<template #options>
    ...
    <!-- TABLE **************************-->
	  ...
	  <!-- modal -->
	  <OuModal mdId="add-record" size="md" title="Add Staff">
		  <CreateRecord :getRecords="getRecords"/>
	  </OuModal>

	  <OuModal mdId="dmk-view-record" size="md" title="Staff Details" :modalSpinner="record_spinner">
		  <RecordView :recordbyId="recordbyId" />
	  </OuModal>

	  <OuModal mdId="dmk-update-record" size="md" title="Update Staff Details" :modalSpinner="record_spinner">
	...
	Swal.fire({
			...
			if (result.isConfirmed) {
				 this.$axios.$delete(`v1/user/delete/${id}`)
					.then((resp) =>{
						if(resp.error == 'false'){
							let swa_msg = 'Staff record deleted successfully';

<template>
  <div>
    ...
    <!-- Page Content -->
    <div class="content">
      <base-block rounded title="Customers" ref="blockLoadingRefresh" class="block-mode-loading-refresh"  header-bg btn-option-fullscreen >
		<template #options>
    ...
    <!-- TABLE **************************-->
        <RecordTable :records="records">
			...
			<td class="mobile-td-expand-120">{{ rs.username }}</td>
			<td>{{ rs.first_name }}</td>
			<td class="">{{ rs.last_name }}</td>
			<td class="text-center"> 
				<div v-if="rs.is_active ==true "><b-icon icon="check-circle-fill" font-scale="1.2" variant="success"></b-icon></div>
				<div v-else><b-icon icon="x-circle-fill" font-scale="1.2" variant="secondary"></b-icon></div>
			 </td>
			<td class="text-center"> 
				<div v-if="rs.is_staff ==true "><b-icon icon="check-circle-fill" font-scale="1.2" variant="success"></b-icon></div>
				<div v-else><b-icon icon="x-circle-fill" font-scale="1.2" variant="secondary"></b-icon></div>
			 </td>
	  ...
	  <!-- modal -->
	  <OuModal mdId="add-record" size="md" title="Add Customer">
		  <CreateRecord :getRecords="getRecords"/>
	  </OuModal>

	  <OuModal mdId="dmk-view-record" size="md" title="Customer Details" :modalSpinner="record_spinner">
		  <RecordView :recordbyId="recordbyId" />
	  </OuModal>

	  <OuModal mdId="dmk-update-record" size="md" title="Update Customer Details" :modalSpinner="record_spinner">
...
export default {
	...
	Swal.fire({
			...
			if (result.isConfirmed) {
				 this.$axios.$delete(`v1/user/delete/${id}`)
					.then((resp) =>{
						if(resp.error == 'false'){
							let swa_msg = 'Customer record deleted successfully';

<template>
  <div>
    ...
    <!-- Page Content -->
    <div class="content">
      <base-block rounded title="GECSS BRANCHES" ref="blockLoadingRefresh" class="block-mode-loading-refresh"  header-bg btn-option-fullscreen >
		<template #options>
    ...
    <!-- TABLE **************************-->
        <RecordTable :records="records">
			...
			<td class="text-center">
				<button v-if="rs.status == 'Active'" class=" btn status-btn btn-success " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
				<button v-else-if="rs.status == 'Agent'" class=" btn status-btn btn-primary " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
				<button v-else-if="rs.status == 'Ongoing'" class=" btn status-btn btn-info " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
				<button v-else-if="rs.status == 'Construction'" class=" btn status-btn btn-success " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
	  ...
	  <!-- modal -->
	  <OuModal mdId="add-record" size="md" title="Add Branch">
		  <CreateRecord :getRecords="getRecords"/>
	  </OuModal>

	  <OuModal mdId="dmk-view-record" size="md" title="Branch Details" :modalSpinner="record_spinner">
		  <RecordView :recordbyId="recordbyId" />
	  </OuModal>

	  <OuModal mdId="dmk-update-record" size="md" title="Update Branch Details" :modalSpinner="record_spinner">
...
export default {
	...
	setLoadingFalse(){
        ...
        async getRecordOnMount(){
            ...
            await this.$axios.$get("v1/branches")
            .then((resp) =>{
                this.records = resp;
            })
       ...
       async getRecordById(id){
		   ...
		   let token = localStorage.getItem('token')
		   this.$axios.defaults.headers.common["Authorization"] = "Token " + token
           await this.$axios.$get(`v1/branch/${id}`)	
            .then((resp) =>{
	...
	Swal.fire({
			...
			if (result.isConfirmed) {
				 this.$axios.$delete(`v1/branch/delete/${id}`)
					.then((resp) =>{
						if(resp.error == 'false'){
	...
	async searchRecords(){
	  ...
	  let token = localStorage.getItem('token')
	 this.$axios.defaults.headers.common["Authorization"] = "Token " + token
       await this.$axios.$get(`v1/branch/search/${this.search.queary}`)
           .then((resp) =>{
			   this.records = resp; 
           })
		   this.setLoadingFalse();

<template>
  <div>
    ...
    <div class="content">
      ...
      <base-block rounded title="Bills and Expenses" ref="blockLoadingRefresh" class="block-mode-loading-refresh"  header-bg btn-option-fullscreen >
		<template #options>
    ...
    <!-- TABLE **************************-->
        <RecordTable :records="records">
			...
			<td>{{ rs.item.slice(0, 30) }}</td>
			<td>{{ rs.quantity }}</td>
			<td><span v-if="rs.price > 0">{{ rs.price.toLocaleString() }}</span> <span v-else>0</span> </td>
			<td> {{ rs.department  }} </td>
			<td>{{ rs.approvedby }}</td>
	  ...
	  <!-- modal -->
	  <OuModal mdId="add-record" size="lg" title="Add Bills & Expenses">
		  <CreateRecord :getRecords="getRecords"/>
	  </OuModal>

	  <OuModal mdId="dmk-view-record" size="lg" title="Bills & Expenses Details" :modalSpinner="record_spinner">
		  <RecordView :recordbyId="recordbyId" />
	  </OuModal>

	  <OuModal mdId="dmk-update-record" size="lg" title="Update Bills & Expenses Details" :modalSpinner="record_spinner">
...
export default {
	...
	setLoadingFalse(){
        ...
        async getRecordOnMount(){
            ...
            await this.$axios.$get("v1/account/expense")
            .then((resp) =>{
                this.records = resp;
       ...
       async getRecordById(id){
		   ...
		   let token = localStorage.getItem('token')
			this.$axios.defaults.headers.common["Authorization"] = "Token " + token
           await this.$axios.$get(`v1/account/expense/${id}`)	
            .then((resp) =>{
	...
	Swal.fire({
			icon: 'warning',
			title: 'Do you want to cancel selected record?',
			showCancelButton: true,
			confirmButtonText: `YES`,
			cancelButtonText:`NO`
			}).then((result) => {
			if (result.isConfirmed) {
				 this.$axios.$post("v1/account/expense/cancel", formData)
					.then((resp) =>{
						if(resp.error == '0'){
							let swa_msg = resp.message
				...
				Swal.fire('Cancellation failed. Pleae try again later', '', 'info')
			}
		})
	  },
	...
	async searchRecords(){
	  ...
	  let token = localStorage.getItem('token')
	  this.$axios.defaults.headers.common["Authorization"] = "Token " + token
       await this.$axios.$get(`v1/account/expense/search/${this.search.queary}`)
           .then((resp) =>{
			   this.records = resp; 
           })
		   this.setLoadingFalse();

<template>
  <div>
    ...
    <!-- Page Content -->
    <div class="content">
      <base-block rounded title="Agent Payments" ref="blockLoadingRefresh" class="block-mode-loading-refresh"  header-bg btn-option-fullscreen >
		<template #options>
    ...
    <!-- TABLE **************************-->
        <RecordTable :records="records">
			...
			<td>{{ rs.agentNo }}</td>
			<td class=""> <span v-if="rs.amount > 0"> {{ rs.amount.toLocaleString() }}</span> <span v-else>0</span> </td>
			<td class="">{{ rs.fromDate }}</td>
			<td class="">{{ rs.toDate }}</td>
			<td class="">{{ rs.createdAt | diffForHumans }}</td>
			<td class="text-center">
				<button v-if="rs.status == 'Paid'" class=" btn status-btn btn-success " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
				<button v-else-if="rs.status == 'unPaid'" class=" btn status-btn btn-primary " type="button" aria-expanded="false">
					{{ rs.status }}
				</button>
			</td>
            <td>
	  ...
	  <!-- modal -->
	  <OuModal mdId="add-record" size="md" title="Add Agent Payments">
		  <CreateRecord :getRecords="getRecords"/>
	  </OuModal>

	  <OuModal mdId="dmk-view-record" size="md" title="Agent Payments Details" :modalSpinner="record_spinner">
		  <RecordView :recordbyId="recordbyId" />
	  </OuModal>

	  <OuModal mdId="dmk-update-record" size="md" title="Update Agent Payments Details" :modalSpinner="record_spinner">
...
export default {
	...
	setLoadingFalse(){
        ...
        async getRecordOnMount(){
            ...
            await this.$axios.$get("v1/hrms/agent_payment")
            .then((resp) =>{
                this.records = resp;
            })
       ...
       async getRecordById(id){
		   ...
		   let token = localStorage.getItem('token')
		   this.$axios.defaults.headers.common["Authorization"] = "Token " + token
           await this.$axios.$get(`v1/hrms/agent_payment/${id}`)	
            .then((resp) =>{
	...
	Swal.fire({
			...
			if (result.isConfirmed) {
				 this.$axios.$delete(`v1/hrms/agent_payment/delete/${id}`)
					.then((resp) =>{
						if(resp.error == 'false'){
							let swa_msg = 'Company branch deleted successfully';
	...
	async searchRecords(){
	  ...
	  let token = localStorage.getItem('token')
	 this.$axios.defaults.headers.common["Authorization"] = "Token " + token
       await this.$axios.$get(`v1/hrms/agent_payment/search/${this.search.queary}`)
           .then((resp) =>{
			   this.records = resp; 
           })
		   this.setLoadingFalse();
